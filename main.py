...
box.grid(row=1,column = 2,columnspan=5)
ttk.Button(root,text='Message to all friends').grid(row=2,column=2)
ttk.Button(root,text='Post to the wall').grid(row=2,column=3,ipady=10)
ttk.Button(root,text='Accept all friend requests').grid(row=2,column=4)
ttk.Button(root,text='Send friend requests to people you may know').grid(row=3,column=2)
ttk.Button(root,text='Get information about selected group').grid(row=3,column=4)

status_bar_right = Text(root, height= 8, width= 100)
status_bar_right.grid(row=5,column= 2,columnspan=5,padx= 5, pady= 10)
scrl = Scrollbar(root, command=status_bar_right.yview)
status_bar_right.config(yscrollcommand=scrl.set)
scrl.grid(row=5, column=5, sticky='n s')
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
