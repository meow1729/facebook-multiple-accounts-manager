...
#ttk.Separator(root,orient=VERTICAL).grid(column=1, rowspan=8, sticky="ns")


var = StringVar(root,value="")
var.set('status of selected group:\n Allegiance selected\n It has 25 ids')
Label(textvariable=var).grid(row=5)
...
box = Text(root,height=8,width=70,pady=10,padx=10,highlightbackground="black",background="#C8C5C9",font = "Helvetica 20 bold")
box.grid(row=1,column = 1,columnspan=5)
ttk.Button(root,text='Message to all friends').grid(row=2,column=1)
ttk.Button(root,text='Post to the wall').grid(row=2,column=2,ipady=10)
ttk.Button(root,text='Accept all friend requests').grid(row=2,column=3)
ttk.Button(root,text='Send friend requests to people you may know').grid(row=3,column=1)
ttk.Button(root,text='Get information about selected group').grid(row=3,column=3)

status_bar_right = Text(root, height= 8, width= 100)
status_bar_right.grid(row=5,column= 1,columnspan=5,padx= 5, pady= 10)
scrl = Scrollbar(root, command=status_bar_right.yview)
status_bar_right.config(yscrollcommand=scrl.set)
scrl.grid(row=5, column=4, sticky='n s')
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
